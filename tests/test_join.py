...
def t1(db: gp.Database):
    ...
    # fmt: on
    return gp.values(rows1, db=db, column_names=["id1", "idd1", "n1"])
...
def t2(db: gp.Database):
    ...
    # fmt: on
    return gp.values(rows2, db=db, column_names=["id2", "idd2", "n2"])
...
def zoo_1(db: gp.Database):
    ...
    # fmt: on
    return gp.values(rows, db=db, column_names=["id", "animal"])
...
def test_join_all_and_all_columns(db: gp.Database, t1: gp.Table, t2: gp.Table):
    row = next(
        iter(t1.join(t2, cond=t1["id1"] == t2["id2"], self_columns={"*"}, other_columns={"*"}))
    )
...
def test_join_no_and_no_columns(db: gp.Database, t1: gp.Table, t2: gp.Table):
    ret = next(iter(t1.join(t2, cond=t1["id1"] == t2["id2"])))
...
def test_join_all_and_no_columns(db: gp.Database, t1: gp.Table, t2: gp.Table):
    ret = next(iter(t1.join(t2, cond=t1["id1"] == t2["id2"], other_columns={"*"})))
...
def test_join_all_and_one_columns(db: gp.Database, t1: gp.Table, t2: gp.Table):
    ret = next(
        iter(t1.join(t2, cond=t1["id1"] == t2["id2"], self_columns={"id1"}, other_columns={"*"}))
    )
...
def test_join_same_column_using(db: gp.Database):
    rows = [(1,), (2,), (3,)]
    t1 = gp.values(rows, db=db, column_names=["id"])
    t2 = gp.values(rows, db=db, column_names=["id"])
    ret = t1.join(t2, using=["id"], self_columns={"id": "t1_id"}, other_columns={"id": "t2_id"})
    for row in ret:
        assert "t1_id" in row and "t2_id" in row


# FIXME: This is expected to have an exception thrown since the results have
