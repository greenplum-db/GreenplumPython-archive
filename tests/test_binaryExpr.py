...
def test_expr_bin_equal_int(db: gp.Database):
    rows = [(1,), (2,), (3,), (2,)]
    t = db.create_dataframe(rows).save_as("temp1", temp=True, column_names=["id"])
...
def test_expr_bin_equal_str(db: gp.Database):
    rows = [("aaa",), ("bbb",), ("ccc",)]
    t = db.create_dataframe(rows).save_as("temp2", temp=True, column_names=["id"])
...
def test_expr_bin_equal_none(db: gp.Database):
    rows = [("aa",), (None,), ("cc",)]
    t = db.create_dataframe(rows).save_as("temp3", temp=True, column_names=["id"])
...
def test_expr_bin_equal_2expr(db: gp.Database):
    rows = [(1,), (2,), (3,)]
    t1 = db.create_dataframe(rows).save_as("temp4", temp=True, column_names=["id"])
    t2 = db.create_dataframe(rows).save_as("temp5", temp=True, column_names=["id"])
...
def test_dataframe_true_div(db: gp.Database):
    nums = db.create_dataframe([(i,) for i in range(1, 10)], column_names=["num"])
...
def test_dataframe_true_div_integers(db: gp.Database):
    nums = db.create_dataframe([(i,) for i in range(4, 5)], column_names=["num"])
...
def test_dataframe_true_div_integer_float(db: gp.Database):
    nums = db.create_dataframe([(i,) for i in range(5, 8, 2)], column_names=["num"])
...
def test_dataframe_true_div_zero(db: gp.Database):
    nums = db.create_dataframe([(i,) for i in range(5)], column_names=["num"])
...
def test_column_in_column(db: gp.Database):
    ...
    t = db.create_dataframe(rows, column_names=["x"])

    rows2 = [(1,), (2,), (3,)]
    t2 = db.create_dataframe(rows2, column_names=["x"])
