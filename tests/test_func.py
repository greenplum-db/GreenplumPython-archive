...
def test_create_func(db: gp.Database):
    ...
    for row in db.apply(lambda: add(1, 2)).rename("result").to_table():
...
def test_create_func_multiline(db: gp.Database):
    ...
    for row in db.apply(lambda: my_max(1, 2)).rename("result").to_table():
...
def test_create_func_tab_indent(db: gp.Database):
	...
	for row in db.apply(lambda: my_min(1, 2)).rename("result").to_table():
...
def test_array_func_group_by_return_composite(db: gp.Database):
    ...
    assert sorted(list(list(ret._fetch())[0].keys())) == sorted(["_sum", "_count", "lab"])
    for row in ret:
        assert row["_sum"] == 3
        assert row["_count"] == 3
...
def test_array_func_group_by_composite_apply(db: gp.Database):
    ...
    assert sorted(list(results._fetch())[0].keys()) == sorted(["sum", "count", "is_even"])
    for row in results:
        assert all(
...
def test_func_return_list_composite(db: gp.Database):
    ...
    results = db.apply(lambda: add_to_cart("alice", ["apple"])).to_table()
    for row in results:
