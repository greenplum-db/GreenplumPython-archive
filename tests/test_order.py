...
def test_order_by_head_desc(db: gp.Database, t: gp.Table):
    ret = t.order_by(t["id"], ascending=False).head(5)
    assert len(list(ret)) == 5
...
def test_order_by_head_operator(db: gp.Database, t: gp.Table):
    ret = t.order_by(t["id"], operator=">").head(3)
...
def test_order_by_multiple_head(db: gp.Database):
    ...
    ret = t.order_by(t["id"]).order_by(t["num"], ascending=False).head(5)
    assert len(list(ret)) == 5
    row = next(iter(ret))
    ...
    ret2 = t.order_by(t["num"], ascending=False).order_by(t["id"]).head(5)
    assert len(list(ret)) == 5
    row = next(iter(ret2))
...
def test_order_by_nulls_last(db: gp.Database):
    ...
    ret = t.order_by(t["year"], nulls_first=False).head(5)
    assert (
