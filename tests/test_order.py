...
def test_order_by_head(db: gp.Database, t: gp.Table):
    ret = list(t.order_by(lambda t: t["id"]).head(5).fetch())
    assert len(ret) == 5
    assert ret[0]["id"] == 0
    assert ret[4]["id"] == 4
...
def test_order_by_head_desc(db: gp.Database, t: gp.Table):
    ret = list(t.order_by(lambda t: t["id"], ascending=False).head(5).fetch())
    assert len(ret) == 5
    assert ret[0]["id"] == 9
    assert ret[4]["id"] == 5
...
def test_order_by_head_operator(db: gp.Database, t: gp.Table):
    ret = list(t.order_by(lambda t: t["id"], operator=">").head(3).fetch())
    assert len(ret) == 3
    assert ret[0]["id"] == 9
    assert ret[2]["id"] == 7
...
def test_order_by_head_asc_operator(db: gp.Database, t: gp.Table):
    with pytest.raises(Exception) as exc_info:
        t.order_by(lambda t: t["id"], ascending=True, operator="<")
...
def test_order_by_multiple_head(db: gp.Database):
    ...
    ret = list(
        t.order_by(lambda t: t["id"]).order_by(lambda t: t["num"], ascending=False).head(5).fetch()
    )
    assert len(ret) == 5
    assert ret[0]["id"] == 1 and ret[0]["num"] == 3
    assert ret[4]["id"] == 3 and ret[4]["num"] == 1
    ret2 = list(
        t.order_by(lambda t: t["num"], ascending=False).order_by(lambda t: t["id"]).head(5).fetch()
    )
    assert len(ret) == 5
...
def test_order_by_nulls_last(db: gp.Database):
    ...
    ret = list(t.order_by(lambda t: t["year"], nulls_first=False).head(5).fetch())
