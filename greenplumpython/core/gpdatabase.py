...
class GPDatabase(SQLDatabase):
    ...
    def get_table(self, table_name, schema=None):
        ...
        db_sql = "select column_name, udt_name from information_schema.columns where table_schema='{schema}' AND table_name='{name}' ORDER BY dtd_identifier".format(
            schema=db_schema, name=table_name
        )
