...
def test_gpapply_case1(db_conn):
    ...
    output_col = [{"a":"int4"}]
    output = GPTableMetadata("basic_output", output_col, distribute_on='RANDOMLY')
    gpApply(table, recsum, data, output, clear_existing=True, runtime_id='plc_r', runtime_type='plpython3u')
...
def test_gpapply_case1_returndata(db_conn):
    ...
    res = gpApply(table, recsum, data, output, '', 'plpython3u')
    assert res.iat[0,0] ==4 or res.iat[1,0] == 4
...
def test_gpapply_case2(db_conn):
    ...
    gpApply(table, aqi_vs_temp, data, output, '', 'plpython3u')
...
def test_gpapply_case2_returndata(db_conn):
    ...
    res = gpApply(table, aqi_vs_temp, data, output, '', 'plpython3u')
    assert res.iat[0,2] == 13.0 or res.iat[0,2] == 6.0
...
def test_gpapply_result_table_distributed_by(db_conn):
    ...
    gpApply(table, recsum2, data, output, '', 'plpython3u')
...
def test_gpapply_result_table_column_num_not_match(db_conn):
    with pytest.raises(Exception) as e:
        ...
        assert gpApply(table, recsum2, data, output, '', 'plpython3u')
...
def test_gpapply_result_table_column_type_not_match(db_conn):
    with pytest.raises(Exception) as e:
        ...
        gpApply(table, recsum3, data, output, '', 'plpython3u')
...
def test_gpapply_pyfunc_error(db_conn):
    with pytest.raises(Exception) as e:
        ...
        output_columns = [{"c1": "int4"},{"c2": "int4"}]
        output = GPTableMetadata("basic_output", output_columns, 'randomly')
        assert gpApply(table, recsumerr, data, output, '', 'plpython3u')
...
def test_view(db_conn):
    ...
    res = gpApply(table_view, inc, data, output, '', 'plpython3u')
...
def test_gpapply_error1_non_table(db_conn):
    with pytest.raises(Exception) as e:
        ...
        output_col = [{"a":"int4"}]
        output = GPTableMetadata("basic_output", output_col, 'randomly')
        assert gpApply(None, recsum, data, output, '', 'plpython3u', True)
...
def test_gpapply_error2_non_func(db_conn):
    with pytest.raises(Exception) as e:
        ...
        output_col = [{"a":"int4"}]
        output = GPTableMetadata("basic_output", output_col, 'randomly')
        assert gpApply(table, None, data, output, '', 'plpython3u', True)
...
def test_gpapply_error3_non_connection(db_conn):
    with pytest.raises(Exception) as e:
        ...
        output_col = [{"a":"int4"}]
        output = GPTableMetadata("basic_output", output_col, 'randomly')
        assert gpApply(table, recsum, None, output, '', 'plpython3u', True)
...
def test_fn_wrong_type(db_conn):
    with pytest.raises(ValueError):
        ...
        output_col = [{"a":"int4"}]
        output = GPTableMetadata("basic_output", output_col, 'randomly')
        assert gpApply(table, 'bad_function', None, output, '', 'plpython3u',True)
...
def test_table_exist(db_conn):
    ...
    gpApply(table, recsum, data, output, '', 'plpython3u', True)
    res = data.execute_query("select * from basic_output")
    assert res.iat[0,0] ==4 or res.iat[1,0] == 4
    ...
    gpApply(table, recsum, data, output, '', 'plpython3u',False)
    res = data.execute_query("select * from basic_output")
    assert res.iat[0,0] ==4 or res.iat[1,0] == 4
    #3 clear.existing is FALSE, when the table exists       (ERROR)
    with pytest.raises(Exception):
        assert gpApply(table, recsum, data, output, '', 'plpython3u', False)
...
def test_additional_junk_params(db_conn):
    ...
    #0 clear.existing is TRUE, when the table doesn't exist (OK)
    data.execute("DROP TABLE IF EXISTS %s;" % output.name)
    gpApply(table, recsum4, data, output, '', 'plpython3u', True, arg1=[12, "int4"], arg2=[13, "int4"])
    res = data.execute_query("select * from basic_output")
...
def test_additional_junk_params_text(db_conn):
    ...
    #0 clear.existing is TRUE, when the table doesn't exist (OK)
    data.execute("DROP TABLE IF EXISTS %s;" % output.name)
    gpApply(table, recsum5, data, output, '', 'plpython3u',True, arg1=[12, "int4"], arg2=["'Hello'", "text"])
    res = data.execute_query("select * from basic_output")
    assert res.iat[0,0] ==20 or res.iat[1,0] == 20
