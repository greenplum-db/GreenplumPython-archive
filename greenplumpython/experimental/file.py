...
def gd() -> dict[str, Any]:
    try:
        return globals["GD"]  # type: ignore reportUnknownVariableType
    except KeyError:
        return sys.modules["plpy"]._GD


@gp.create_function
def _dump_file_chunk(tmp_archive_name: str, chunk_base64: str) -> str:
    server_tmp_dir_handle = f"__pygp_{uuid.uuid4().hex}"
    if server_tmp_dir_handle not in gd():
        server_tmp_dir = tempfile.TemporaryDirectory()
        gd()[server_tmp_dir_handle] = server_tmp_dir  # Pin to GD for later UDFs
    else:
        server_tmp_dir = gd()[server_tmp_dir_handle]

    server_tmp_dir_path: pathlib.Path = pathlib.Path(server_tmp_dir.name)
    ...
    return server_tmp_dir_handle


@gp.create_function
def _remove_tmp_dir(server_tmp_dir_handle: str) -> None:
    gd()[server_tmp_dir_handle].cleanup()


@gp.create_function
def _extract_files(server_tmp_dir_handle: str, tmp_archive_name: str, returning: str) -> list[str]:
    server_tmp_dir_path: pathlib.Path = pathlib.Path(gd()[server_tmp_dir_handle].name)
    tmp_archive_path = server_tmp_dir_path / f"{tmp_archive_name}.tar.gz"
...
def _archive_and_upload(tmp_archive_name: str, files: list[str], db: gp.Database) -> str:
    with tempfile.TemporaryDirectory() as local_tmp_dir:
        ...
        with psycopg2.connect(db._dsn, options=server_options) as util_conn:  # type: ignore reportUnknownVariableType
            with util_conn.cursor() as cursor:  # type: ignore reportUnknownVariableType
                cursor.execute(
                    f"CREATE TEMP TABLE {tmp_archive_name} (id serial, text_base64 text);"
                )
...
def _from_files(_, files: list[str], parser: NormalFunction, db: gp.Database) -> gp.DataFrame:
    ...
    server_tmp_dir_handle = _archive_and_upload(tmp_archive_name, files, db)
    func_sig = inspect.signature(parser.unwrap())
    result_members = get_type_hints(func_sig.return_annotation)
    df = db.apply(
        lambda: parser(_extract_files(server_tmp_dir_handle, tmp_archive_name, "files")),
        expand=len(result_members) == 0,
    )
    db.apply(lambda: _remove_tmp_dir(server_tmp_dir_handle))
    return df
...
@gp.create_function
def _install_on_server(server_tmp_dir_handle: str, local_tmp_dir: str, requirements: str) -> str:
...
def _install_packages(db: gp.Database, requirements: str):
    ...
    with tempfile.TemporaryDirectory() as local_tmp_dir:
        ...
        server_tmp_dir_handle = _archive_and_upload(tmp_archive_name, [local_tmp_dir], db)
        extracted = db.apply(lambda: _extract_files(tmp_archive_name, "root"))
        assert len(list(extracted)) == 1
        installed = extracted.apply(
            lambda _: _install_on_server(server_tmp_dir_handle, local_tmp_dir.name, requirements)
        )
